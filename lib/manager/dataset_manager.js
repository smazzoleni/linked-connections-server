...
class DatasetManager {
    ...
    async processStaticGTFS(index, dataset) {
        ...
        try {
            ...
            if (file_name != null) {
                ...
                let converter = new Connections(uncompressed_feed, { store: 'LevelStore' });

                let connSources = await converter.getConnectionsByZones();
                let zones = Object.keys(connSources);
                ...
                zones.forEach(zone => {
                    ...
                    let fileWriter = fs.createWriteStream(this.storage + '/linked_connections/' + companyName + '/'
                        + file_name + '/' + zone + '_tmp.jsonld', 'utf8');
                    logger.info('Creating ' + companyName + ' Linked Connections for ' + zone + ' zone...');
                    connSources[zone].pipe(new Connections2JSONLD(baseURIs))
                        ...
                        .on('finish', async () => {
                            ...
                            logger.info("Sorting " + companyName + " Linked Connections of " + zone + " by departure time...");
                            let sorted_path = await this.sortLCByDepartureTime(this.storage + '/linked_connections/' + companyName + '/'
                                + file_name + '/' + zone + '_tmp.jsonld', companyName, file_name, zone);

                            logger.info('Fragmenting ' + companyName + ' Linked Connections for ' + zone + ' zone...');
                            // Delete unsorted Linked Connections graph
                            del([this.storage + '/linked_connections/' + companyName + '/' + file_name + '/' + zone + '_tmp.jsonld'], { force: true });
                            // Create folder for Linked Connection fragments
                            fs.mkdirSync(this.storage + '/linked_pages/' + companyName + '/' + file_name + '/' + zone);
                            ...
                            reader.pipe(new jsonldstream.Deserializer())
                                .pipe(new pageWriterStream(this.storage + '/linked_pages/' + companyName + '/'
                                    + file_name + '/' + zone, dataset.fragmentSize || 50000))
                                .on('finish', async () => {
                                    logger.info('Compressing ' + companyName + ' Linked Connections fragments for ' + zone + ' zone...');
                                    child_process.spawn('gzip', [zone + '.jsonld'], {
                                        cwd: this.storage + '/linked_connections/' + companyName + '/' + file_name,
                                        stdio: 'ignore'
                                    });
                                    let comp = child_process.spawn('find', ['.', '-type', 'f', '-exec', 'gzip', '{}', '+'], {
                                        cwd: this.storage + '/linked_pages/' + companyName + '/' + file_name + '/' + zone,
                                        stdio: 'ignore'
                                    });
                                    comp.on('close', code => {
                                        console.log('Finished for ' + zone + ' zone!!');
                                        finished();
                                    });
                                });
                        });
    ...
    sortLCByDepartureTime(path, company, file_name, zone) {
        return new Promise((resolve, reject) => {
            child_process.exec('sort -T ' + this.storage + '/tmp/ -t \\" -k 17 ' + this.storage + '/linked_connections/'
                + company + '/' + file_name + '/' + zone + '_tmp.jsonld > ' + this.storage + '/linked_connections/' + company
                + '/' + file_name + '/' + zone + '.jsonld',
                (err, stdout, stderr) => {
                    if (err) {
                        logger.error(err);
                        reject();
                    }
                    resolve(this.storage + '/linked_connections/' + company + '/' + file_name + '/' + zone + '.jsonld');
                });
        });
    }
