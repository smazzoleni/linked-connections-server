...
class DatasetManager {
    ...
    async processStaticGTFS(index, dataset) {
        ...
        try {
            ...
            if (file_name != null) {
                ...
                // Load the fragment index if any
                let fragmentIndex = null;
                if (dataset.fragmentIndex) {
                    fragmentIndex = new Map(JSON.parse(fs.readFileSync(dataset.fragmentIndex, 'utf8')));
                }
                ...
                let converter = new Connections(uncompressed_feed, {
                    format: 'jsonld',
                    store: 'KeyvStore',
                    baseUris: baseURIs,
                    fragmentBy: dataset.fragmentBy,
                    fragmentIndex: fragmentIndex
                });

                fs.mkdirSync(this.storage + '/linked_connections/' + companyName + '/' + file_name);
                ...
                let finish = () => {
                    ...
                    if (count >= connSources.length) {
                        ...
                        // GTFS feed completed successfuly, proceed to delete .lock file and temporal conversion files
                ...
                let connSources = await converter.map();
                
                for (let csource of connSources) {
                    let sourceName = (csource.split('/')[csource.split('/').length - 1]).split('.')[0];
                    let sorted_path = this.storage + '/linked_connections/' + companyName + '/' + file_name + '/' + sourceName + '.jsonld';
                    logger.info('Sorting ' + companyName + ' (' + sourceName + ' zone) Linked Connections by departure time...');
                    await this.sortLCByDepartureTime(csource, sorted_path);
                    logger.info(sourceName + ' zone sorted for ' + companyName);
                    logger.info('Fragmenting ' + companyName + ' Linked Connections for ' + sourceName + ' zone...');
                    // Create folder for Linked Connection fragments
                    fs.mkdirSync(this.storage + '/linked_pages/' + companyName + '/' + file_name + '/' + sourceName);
                    ...
                    reader.pipe(new jsonldstream.Deserializer())
                        .pipe(new pageWriterStream(this.storage + '/linked_pages/' + companyName + '/'
                            + file_name + '/' + sourceName, dataset.fragmentSize || 50000))
                        .on('finish', async () => {
                            logger.info('Compressing ' + companyName + ' Linked Connections fragments for ' + sourceName + ' zone...');
                            child_process.spawn('gzip', [sourceName + '.jsonld'], {
                                cwd: this.storage + '/linked_connections/' + companyName + '/' + file_name,
                                stdio: 'ignore'
                            });
                            let comp = child_process.spawn('find', ['.', '-type', 'f', '-exec', 'gzip', '{}', '+'], {
                                cwd: this.storage + '/linked_pages/' + companyName + '/' + file_name + '/' + sourceName,
                                stdio: 'ignore'
                            });
                            comp.on('close', code => {
                                logger.info('Finished creating Linked Connections for ' + companyName + ' in ' + sourceName + ' zone!!');
                                finish();
                            });
                        });
                }
    ...
    sortLCByDepartureTime(sourcePath, outputPath) {
        return new Promise((resolve, reject) => {
            child_process.exec('sort -T ' + this.storage + '/tmp/ -t \\" -k 17 ' + sourcePath + ' > ' + outputPath,
                (err, stdout, stderr) => {
                    if (err) {
                        logger.error(err);
                        reject();
        ...
        return Promise.all(this._datasets.map(async dataset => {
            if (fs.existsSync(this.storage + '/datasets/' + dataset.companyName)) {
                ...
                if (incomplete !== null) {
                    ...
                    this.storage + '/linked_connections/' + dataset.companyName + '/' + incomplete,
