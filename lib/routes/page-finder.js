...
router.get('/:agency/:zone/connections', async (req, res) => {
    let t0 = new Date();
    ...
    if (req.url.indexOf('connections/') >= 0) {
        res.location('/' + agency + '/' + zone + '/' + 'connections?departureTime=' + departureTime.toISOString());
        res.status(302).send();
        return;
    }

    // Check if there is data for the requested company
    if (!utils.staticFragments[agency]) {
        res.status(404).send("Agency not found on server's data");
        return;
    }
    ...
    try {
        ...
        if (req.headers['accept-datetime'] !== undefined) {
            ...
            // Check if required zone is defined for the closest version
            if (!utils.staticFragments[agency][sortedVersions[0]][zone]) {
                res.status(404).send("Zone " + zone + " not found in server's data");
                return;
            }
            // Find closest resource to requested version 
            let closest_version = utils.findResource(agency, departureTime.getTime(), sortedVersions, zone);
            // Set Memento headers pointng to the found version
            res.location('/memento/' + agency + '/' + zone + '?version=' + closest_version[0] + '&departureTime='
                + new Date(closest_version[1]).toISOString());
            res.set({
                'Vary': 'Accept-Encoding, Accept-Datetime',
                'Link': '<' + host + agency + '/' + zone + '/connections?departureTime=' + departureTime.toISOString() + '>; rel=\"original timegate\"'
            });
        ...
        // Check if required zone exists on last version
        if (!utils.staticFragments[agency][versions[versions.length - 1]][zone]) {
            res.status(404).send("Zone " + zone + " not found in server's data");
            return;
        }
        ...
        t0 = new Date();
        let [static_version, found_fragment, index] = utils.findResource(agency, departureTime.getTime(), sorted_versions, zone);
        ...
        if (departureTime.getTime() !== found_fragment) {
            res.location('/' + agency + '/' + zone + '/connections?departureTime=' + ff.toISOString());
            res.status(302).send();
            return;
        }

        let sf_path = storage + '/linked_pages/' + agency + '/' + static_version + '/' + zone + '/';
        let rt_exists = false;
        let lowLimit = found_fragment;
        let highLimit = utils.staticFragments[agency][static_version][zone][index + 1];
