...
class Catalog {
    ...
    async createCatalog() {
        let catalog = {
            ...
            "@id": server_config.protocol + "://" + server_config.hostname + "/catalog",
            "@type": "dcat:Catalog", 
        ...
        await Promise.all(datasets.map(async dataset => {
            ...
            if(fs.existsSync(storage + '/linked_pages/' + dataset.companyName) 
                ...
                let dcatDataset = {
                    "@id": server_config.protocol + "://" + server_config.hostname + "/" + dataset.companyName,
                    "@type": "dcat:Dataset",
        ...
        if(dataset.geographicArea.subAreas && dataset.geographicArea.subAreas.length > 0) {
            ...
            for(let v in versions) {
                for(let sa in subAreas) {
                    // Check there is data for this sub area
                    if(fs.existsSync(storage + '/linked_pages/' + dataset.companyName + '/' + versions[v] + '/' + subAreas[sa]['name'])) {
                        ...
                        let dist = {
                            "@id": server_config.protocol + "://" + server_config.hostname + "/" + dataset.companyName + "/" + subAreas[sa]['name']
                                + "/" + versions[v],
                            "@type": "dcat:Distribution",
                            "dcat:accessURL": server_config.protocol + "://" + server_config.hostname + "/" + dataset.companyName + "/" 
            ...
            for(let v in versions) {
                ...
                let dist = {
                    "@id": server_config.protocol + "://" + server_config.hostname + "/" + dataset.companyName + "/" + versions[v],
                    "@type": "dcat:Distribution",
                    "dcat:accessURL": server_config.protocol + "://" + server_config.hostname + "/" + dataset.companyName + "/connections",
                    "dct:license": "http://creativecommons.org/publicdomain/zero/1.0/",
                    "dcat:mediaType": "application/ld+json",
                    "dct:issued": versions[v],
                    "schema:startDate": start,
