...
router.get('/:agency/:zone', async (req, res) => {
    ...
    try {
        let sf_path = storage + '/linked_pages/' + agency + '/' + version + '/' + zone + '/' + resource + '.jsonld.gz';
        t0 = new Date();
        ...
        let low_index = (utils.staticFragments[agency][version][zone]).indexOf(lowLimit);
        let highLimit = utils.staticFragments[agency][version][zone][low_index + 1];
